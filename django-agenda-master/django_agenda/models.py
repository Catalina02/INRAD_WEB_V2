...
1. Availabilities están destinados a que los usuarios creen y editen. Ellos indican
    cuando un usuario o elemento no está ocupado. Las disponibilidades pueden superponerse, pero se superponen
    no significa que estén más disponibles. 

2. AvailabilityOccurrences son un modelo de datos de uso interno utilizado principalmente
    para mejorar el rendimiento. Indican tiempo libre. 


3. Time Slots son las que se crean cuando se crean las reservas. Un momento
    la ranura puede estar ocupada o libre, y pueden superponerse. 
    
Una hora esta libre si hay una AvailabilityOccurrences libre y no hay Time Slots ocupados 

En efecto, las Availabilities representan la intención del usuario y los intervalos de tiempo
representan lo que realmente está programado para suceder. 
...
# this moved in Django 3
FieldDoesNotExist = (
    exceptions.FieldDoesNotExist
    if hasattr(exceptions, "FieldDoesNotExist")
    else models.FieldDoesNotExist
)
...
class TimeSlotMeta(Meta):
    def __new__(mcs, name, bases, attrs):
        ...
        if not meta.abstract:
            related_name = "time_slots"
            field_name = TimeSlotMeta.get_booking_field(model)
            booking_cls = TimeSlotMeta.get_booking_model(model)
            try:
                meta.get_field(field_name)
            except FieldDoesNotExist:
                model.add_to_class(
...
class AbstractAvailability(models.Model, metaclass=Meta):
    ...
    def recreate_occurrences(self, start: datetime, end: datetime):
        """
         Recrear todas las ocurrencias de disponibilidad entre el inicio y el final

         Está destinado a utilizarse cuando se guarda una disponibilidad. 
        """
